...
// The footer's column headings are h2 in the markup; the selector has to say so.
check('footer heading selector', css.includes('.footer-grid h2'));
for (const p of pages) {
  const footer = readFileSync(p, 'utf8').match(/footer-grid[\s\S]*?footer-bottom/);
  if (footer) check(`${p} footer headings`, !/<h3[ >]/.test(footer[0]), 'markup drifted to h3');
}

// Feedback on press, not only on hover: a touch screen has no hover at all.
check('press states', css.includes('.svc-card:active'));
